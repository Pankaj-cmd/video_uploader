...
const [videoSrc , setVideoSrc] = useState(videosArr[0])
  ...
  return <Stack direction={['column','row']} h={'100vh'}>
    <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} w={'full'} p={'8'} overflow={'auto'}>
            <Heading>Sample Video</Heading>
            <Text>Description:-This is sample video</Text>
        </VStack>

    </VStack>
    <VStack w={['full','xl']} alignItems={'stretch'}
    p={'8'} spacing={'8'} overflow={'auto'}
    >
    {
        videosArr.map((item,index)=>(
             <Button variant={'ghost'} onClick={()=>setVideoSrc(item)} colorScheme={'purple'}>Sample Vidoe {index+1}</Button>
             ))
    }
